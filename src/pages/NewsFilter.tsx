...
export function NewsFilter() {
  ...
  // 핵심: 탭 이름(q)을 서버로 보내서 코인니스 내에서 검색해오게 합니다.
  ...
  // 탭이 바뀔 때마다 자동으로 서버에 '검색' 명령을 내립니다.
  useEffect(() => {
    fetchNews(activeTab)
  }, [activeTab])
    ...
    <div className="container mx-auto max-w-4xl px-4 py-6 text-zinc-100">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-black font-sans">Trading Room Control</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => fetchNews(activeTab)} className="p-2 border border-zinc-800 rounded-2xl hover:bg-zinc-900 transition-all">
      ...
      <div className="space-y-4 pb-32">
        {isLoading && activeTab !== "BOOKMARKS" ? (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <p className="text-zinc-600 font-bold text-sm tracking-widest animate-pulse font-sans uppercase">Searching Coinness...</p>
          </div>
          ...
          <div className="text-center py-24 text-zinc-700 border border-zinc-900 rounded-[40px] border-dashed px-6">
            <p className="font-medium italic leading-relaxed">
              {activeTab === "BOOKMARKS" ? "아직 저장된 뉴스가 없습니다." : `"${activeTab}" 관련 소식을 코인니스에서 찾을 수 없습니다.`}
            </p>
          </div>
        ) : (
            ...
            return (
              <div key={i} className="bg-zinc-900/30 border border-zinc-800 p-7 rounded-[32px] hover:border-zinc-600 transition-all group">
                <div className="flex justify-between items-center mb-5">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-orange-500 font-black bg-orange-500/10 px-3 py-1.5 rounded-full font-sans">Insight Feed</span>
                ...
                <h3 className="text-xl font-bold leading-snug mb-6 group-hover:text-white transition-colors tracking-tight">{item.title}</h3>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md flex items-center justify-center p-6 z-50">
          <div className="bg-zinc-950 border border-zinc-800 p-10 rounded-[48px] w-full max-w-md shadow-2xl">
            ...
            <div className="flex gap-3 mb-10">
              ...
              />
              <button 
                onClick={() => { if(newKeyword && !keywords.includes(newKeyword)) { setKeywords([...keywords, newKeyword]); setNewKeyword(""); } }}
                className="bg-orange-500 hover:bg-orange-600 px-6 rounded-2xl font-black transition-all active:scale-90"
              ><Plus /></button>
            </div>
            ...
            <button 
              ...
              className="w-full mt-10 bg-white text-zinc-950 py-5 rounded-[24px] font-black text-lg hover:bg-orange-500 transition-all"
            >APPLY</button>
          </div>
        </div>
      )}
