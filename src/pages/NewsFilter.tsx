...
export function NewsFilter() {
  ...
  // ⭐ 띄어쓰기 무시 필터링 로직 (아서 헤이즈 vs 아서헤이즈 모두 해결)
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-6 text-zinc-100">
      ...
      <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
        <button onClick={() => setActiveTab("ALL")} className={`px-6 py-2.5 rounded-full border text-sm font-bold transition-all whitespace-nowrap ${activeTab === 'ALL' ? 'bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-500/20' : 'border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}>전체 속보</button>
        ...
        {keywords.map(kw => (
          <button key={kw} onClick={() => setActiveTab(kw)} className={`px-6 py-2.5 rounded-full border text-sm font-bold transition-all whitespace-nowrap ${activeTab === kw ? 'bg-zinc-100 border-white text-zinc-950 shadow-lg' : 'border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}>{kw}</button>
        ))}
      </div>
      ...
      <div className="space-y-4 pb-32">
        {isLoading && activeTab !== "BOOKMARKS" ? (
          <div className="text-center py-32">
            ...
            <p className="text-zinc-600 font-bold tracking-widest text-xs uppercase">Connecting to Coinness...</p>
          </div>
        ) : filteredNews.length === 0 ? (
          <div className="text-center py-24 text-zinc-700 border border-zinc-900 rounded-[40px] border-dashed px-10">
            <p className="font-medium italic leading-relaxed">
              {activeTab === "BOOKMARKS" ? "저장된 뉴스가 없습니다." : `"${activeTab}" 관련 속보가 현재 목록에 없습니다.`}
            </p>
          </div>
        ) : (
          filteredNews.map((item: any, i: number) => {
            ...
            return (
              <div key={i} className="bg-zinc-900/30 border border-zinc-800 p-7 rounded-[32px] hover:border-zinc-600 transition-all group active:scale-[0.98]">
                <div className="flex justify-between items-center mb-5">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-800/50 rounded-full border border-zinc-700/50">
                    ...
                    <span className="text-[11px] text-zinc-300 font-bold">{item.timestamp}</span>
                  </div>
                  <button 
                    onClick={() => {
                ...
                <h3 className="text-xl font-bold leading-snug mb-6 group-hover:text-white transition-colors tracking-tight">
                  {item.title}
                </h3>
                <a href={item.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-bold text-zinc-500 hover:text-orange-500 transition-colors uppercase tracking-widest">
